...
svaba <- as.data.table(fread(svabaVCF,header=TRUE,sep = "\t",stringsAsFactors = FALSE,na.strings=c(".", "NA")))
svaba <- cbind(Sample = tumId, SV.id = 1:nrow(svaba), svaba)
svaba$start_1=svaba$start1
svaba$start_2=svaba$start2
#added 2022.05.02
svaba[is.na(SPAN), SPAN := start_2-start_1]
svaba[chromosome_1 != chromosome_2, SPAN := -1]
#print(svaba)

## get SV events that pass filter - intra and interchr events ##
# fold-back inversion 
#maxfoldBackInvAD is not being used! --added 2022.05.02
#maxfoldBackInvAD <- svaba[SPAN > 0 & SPAN < maxFBISPAN & orient_1==orient_2, quantile(AD, 0.95)]

indFBI <- svaba[(SPAN > 0 & SPAN < maxFBISPAN & orient_1 == orient_2), which=TRUE]
# use minSPAN or maxFBISPAN
indSVABA <- svaba[ ((SPAN >= minSPAN ) | SPAN == -1), which=TRUE]
save.image(outImage)
...
svaba[orient_1=="rev" & orient_2=="fwd" & SPAN > minColSPAN, type := "Deletion"]
svaba[orient_1=="fwd" & orient_2=="rev" & SPAN > minColSPAN, type := "Duplication"]
svaba[(orient_1==orient_2) & SPAN > 0, type := "Inversion"]
...
svaba[SPAN == -1, type := "InterChr"]
svaba[SPAN > minTrans, type := "Translocation"]
...
sv[type == "InterChr" & !SV.id %in% consensus.sv.id, CN_overlap_type := NA] 
