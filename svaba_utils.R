...
getSVfromCollapsedVCF <- function(vcf, chrs = c(1:22, "X"), genomeStyle = "NCBI"){
  #message("testing in getSVfromCollapsedVCF ",class(vcf))
	if (class(vcf) == "CollapsedVCF"){
			svGR <- as.data.frame(rowRanges(vcf))

      #message("test svGR",svGR)
			svGR$ALT <- unlist(svGR$ALT)
      # message("matesvGR$ALT...testing",svGR$ALT)
			bkpt2 <- str_match(svGR$ALT, "([chr0-9XYA-Z.]+):(\\d+)")[, 2:3]
      
			mateID <- info(vcf)$MATEID
      # id <- info(vcf)$ID
      # message("dimensions of the new dataframe, taken from vcf: ", length(mateID))

      # message("mateID....testing",mateID)
			...
			## get read support ##, deprecated, only used for 10x barcodes...
			...
			# DP - READ DEPTH
			if (sum(rownames(geno(header(vcf))) %in% c("DP")) > 0){
				sv <- cbind(sv, DP = geno(vcf)$DP[,ncol(geno(vcf)$DP)])
			}
			# GT - filter out 1/1
      ...
      #remove lines with mateID equals to NA -- added 
      sv <- sv[!is.na(sv$mateID),]

      # ## get breakpoint 2 alt ##
      # #assign new ID -- added
      rows <- rownames(sv)[unlist(lapply(sv$mateID, function(x) which(grepl(x, rownames(sv)))))]
      subset_sv <- sv[rownames(sv) %in% rows,]
      # rows <- sv[rownames(sv)[unlist(lapply(sv$mateID, function(x) which(grepl(x, rownames(sv)))))]]
      sv <- subset_sv
      #sv$mateID <- rownames(sv)
...
#	sv <- sv[grepl(":1", rownames(sv)), ] ## keep 1st mate in pair 
  sv <- sv[grepl(":1|o", rownames(sv)), ] ## keep 1st mate in pair modified 2022.04.20
...
###########################################
loadVCFtoDataTableByChromosome <- function(svFiles, chrs = as.character(c(1:22, "X")), genomeStyle = "NCBI", genomeBuild = "hg38", minSPAN = 10000, minSPANBX = 10000, minBXOL = 0, maxBXOL = Inf, maxBXCount = 10000, minBSDsupport = 5, dupSV.bpDiff = 1000){
  seqlevelsStyle(chrs) <- genomeStyle
  message("testseqlevelsStyle ",genomeStyle)
  if (length(svFiles) > 0){ ## sv vcf file exists ##
      vcf <- tryCatch({
        message("loading vcf ", svFiles)
        readVcf(svFiles, genome = genomeBuild) # create vcf object here... what format does readVcf end up in?
      }, error = function(x){ 
        return(NA) 
      })
  }
  # message("before fail tesing....")
...
filterSVABA <- function(x, minSPAN = 10000, dupSV.bpDiff = 1000, minBXOL = 0){
  sv <- copy(x)
  ## filter by BXOL support ##
  sv$support <- NA
...
getSVType <- function(sv, minColSPAN = 10000, minTrans = 1e7){#, maxInvSPAN = 1e6, maxFBISPAN = 30000){
	...
	#sv$type[sv$orient_1=="rev" & sv$orient_2=="fwd" & sv$SPAN > minColSPAN]= "Deletion"
    ...
    if (nrow(dataByChr) > 0){
      ...
      if (!is.null(xlim)){ 
        ...
        end1 <- dataByChr[i,"start_2"]
					...
					## breakpoint type ##
          # TANDEM DUPLICATION #
#					if (dataByChr[i, "getSVType"] == "fwd" && dataByChr[i, "orient_2"] == "rev"){
